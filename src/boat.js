...
import { pirateFlagTexture } from './textures.js';
...
/**
 * O casco é montado anel por anel: assim as faixas de cor (azul claro, listra
 * branca e azul escuro) ficam retas e nítidas, como no barco do desenho.
 */
function createHull() {
  ...
  hull.add(
    new THREE.Mesh(geometry, [
      flat(PALETTE.hullBlue, { side: THREE.DoubleSide }),
...
/** Polly, a papagaia verde empoleirada perto da proa. */
function createParrot() {
  ...
  const body = mesh(new THREE.SphereGeometry(0.22, 10, 8), flat(0x3fc94f));
  body.scale.set(0.8, 1.25, 0.8);
  bird.add(body);
  bird.add(mesh(new THREE.SphereGeometry(0.16, 10, 8), flat(0x3fc94f), 0, 0.24, 0));
  const beak = mesh(new THREE.ConeGeometry(0.09, 0.22, 8), flat(PALETTE.flowerYellow), 0, 0.24, 0.16);
  beak.rotation.x = Math.PI / 2;
  bird.add(beak);
  bird.add(mesh(new THREE.SphereGeometry(0.045, 6, 5), flat(PALETTE.eyeBlack), 0.07, 0.3, 0.12));
  bird.add(mesh(new THREE.SphereGeometry(0.045, 6, 5), flat(PALETTE.eyeBlack), -0.07, 0.3, 0.12));
  const crest = mesh(new THREE.ConeGeometry(0.07, 0.18, 6), flat(0x2f9fd8), 0, 0.44, -0.02);
  bird.add(crest);
  for (const sx of [-1, 1]) {
    const wing = mesh(new THREE.SphereGeometry(0.13, 8, 6), flat(PALETTE.flowerRed), sx * 0.16, -0.02, -0.05);
...
/** Barco do Vovô Pig: casco azul, cabine de madeira, bandeira pirata e tripulação. */
export class Boat {
  constructor(scene) {
    ...
    // Mastro com a bandeira pirata.
    const mast = new THREE.Group();
    mast.position.set(0, DECK, 1.15);
    mast.add(mesh(new THREE.CylinderGeometry(0.075, 0.09, 3.6, 8), flat(0xf7f9fc), 0, 1.8, 0));
    mast.add(mesh(new THREE.SphereGeometry(0.16, 10, 8), flat(PALETTE.flowerRed), 0, 3.66, 0));
    const flagMaterial = new THREE.MeshLambertMaterial({
      map: pirateFlagTexture(),
    ...
    // Tripulação fixa: Vovô Pig no leme e Vovó Pig no convés.
    this.crew = [];
  ...
  update(dt, input, time, islands) {
    ...
    for (const island of islands) {
      ...
      const minDistance = data.radius + 2.4;
